...
get_rows($H,$h=null,$n="<p class='error'>"){global$g;$vb=(is_object($h)?$h:$g);$J=array();$I=$vb->query($H);if(is_object($I)){while($K=$I->fetch_assoc())$J[]=$K;}elseif(!$I&&!is_object($h)&&$n&&defined("PAGE_HEADER"))echo$n.adminer_error()."\n";return$J;}function
...
adminer_redirect($A,$se=null){if($se!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($A!==null?$A:$_SERVER["REQUEST_URI"]))][]=$se;}if($A!==null){if($A=="")$A=".";header("Location: $A");exit;}}function
query_redirect($H,$A,$se,$ag=true,$xc=true,$Ec=false,$vh=""){global$g,$n,$b;if($xc){$Vg=microtime(true);$Ec=!$g->query($H);$vh=adminer_format_time($Vg);}$Tg="";if($H)$Tg=$b->messageQuery($H,$vh);if($Ec){$n=adminer_error().$Tg;return
false;}if($ag)adminer_redirect($A,$se.$Tg);return
true;}function
queries($H){global$g;static$Uf=array();static$Vg;if(!$Vg)$Vg=microtime(true);if($H===null)return
array(implode("\n",$Uf),adminer_format_time($Vg));$Uf[]=(preg_match('~;$~',$H)?"DELIMITER ;;\n$H;\nDELIMITER ":$H).";";return$g->query($H);}function
...
switch_lang(){global$ca,$Ud;echo"<form action='' method='post'>\n<div id='lang'>",lang(19).": ".html_select("lang",$Ud,$ca,"this.form.submit();")," <input type='submit' value='".lang(20)."' class='hidden'>\n","<input type='hidden' name='token' value='".get_token()."'>\n";echo"</div>\n</form>\n";}if(isset($_POST["lang"])&&verify_token()){cookie("adminer_lang",$_POST["lang"]);$_SESSION["lang"]=$_POST["lang"];$_SESSION["translations"]=array();adminer_redirect(remove_from_uri());}$ca="en";if(isset($Ud[$_COOKIE["adminer_lang"]])){cookie("adminer_lang",$_COOKIE["adminer_lang"]);$ca=$_COOKIE["adminer_lang"];}elseif(isset($Ud[$_SESSION["lang"]]))$ca=$_SESSION["lang"];else{$ua=array();preg_match_all('~([-a-z]+)(;q=([0-9.]+))?~',str_replace("_","-",strtolower($_SERVER["HTTP_ACCEPT_LANGUAGE"])),$ke,PREG_SET_ORDER);foreach($ke
...
adminer_error(){global$g;$J=h($g->error);if(preg_match('~^(.*\\n)?([^\\n]*)\\n( *)\\^(\\n.*)?$~s',$J,$B))$J=$B[1].preg_replace('~((?:[^&]|&[^;]*;){'.strlen($B[3]).'})(.*)~','\\1<b>\\2</b>',$B[2]).$B[4];return
nl_br($J);}function
...
array();}function
db_collation($m,$ob){}function
tables_list(){global$g;$J=array();foreach(sdb_request_all('ListDomains','DomainName')as$R)$J[(string)$R]='table';if($g->error&&defined("PAGE_HEADER"))echo"<p class='error'>".adminer_error()."\n";return$J;}function
...
as$X){list($lb,$We,$X)=explode(" ",$X,3);if($lb=="_id")$Gb["query"]["ids"]["values"][]=$X;elseif($lb.$X!=""){$rh=array("term"=>array(($lb!=""?$lb:"_all")=>$X));if($We=="=")$Gb["query"]["filtered"]["filter"]["and"][]=$rh;else$Gb["query"]["filtered"]["query"]["bool"]["must"][]=$rh;}}if($Gb["query"]&&!$Gb["query"]["filtered"]["query"]&&!$Gb["query"]["ids"])$Gb["query"]["filtered"]["query"]=array("match_all"=>array());$Vg=microtime(true);$zg=$this->_conn->query($H,$Gb);if($Mf)echo$b->selectQuery("$H: ".print_r($Gb,true),adminer_format_time($Vg));if(!$zg)return
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="',lang(48),'">AI</acronym>',adminer_doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td id="label-default">',lang(49),(support("comment")?"<td id='label-comment'".($tb?"":" class='hidden'").">".lang(47):"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.3.1' alt='+' title='".lang(99)."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($Zh){if(!$O)adminer_redirect($A);queries_redirect($A,lang(159),$Xb->update($a,$O,$Wf,!$Uh));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$I=$Xb->insert($a,$O);$Wd=($I?last_id():0);queries_redirect($A,lang(160,($Wd?" $Wd":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($p
...
as$m){if(count($l)==1||$m!=""){if(!create_database($m,$K["collation"]))$ah=false;$Vd=$m;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($Vd),lang(177),$ah);}}else{if(!$K["collation"])adminer_redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),lang(178));}}page_header(DB!=""?lang(64):lang(106),$n,array(),h(DB));$ob=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$ob);elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$ad){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ad,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($ob?html_select("collation",array(""=>"(".lang(92).")")+$ob,$K["collation"]).adminer_doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
...
adminer_redirect($_);}}page_header($_GET["ns"]!=""?lang(65):lang(66),$n);if(!$K)$K["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
<p>
',lang(94),': ',html_select("on_delete",array(-1=>"")+explode("|",$Te),$K["on_delete"]),' ',lang(93),': ',html_select("on_update",array(-1=>"")+explode("|",$Te),$K["on_update"]),adminer_doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="',lang(14),'">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;$kf="VIEW";if($x=="pgsql"&&$a!=""){$P=table_status($a);$kf=strtoupper($P["Engine"]);}if($_POST&&!$n){$C=trim($K["name"]);$Ga=" AS\n$K[select]";$A=ME."table=".urlencode($C);$se=lang(192);$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$x!="sqlite"&&$U=="VIEW"&&$kf=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ga,$A,$se);else{$qh=$C."_adminer_".uniqid();drop_create("DROP $kf ".table($a),"CREATE $U ".table($C).$Ga,"DROP $U ".table($C),"CREATE $U ".table($qh).$Ga,"DROP $U ".table($qh),($_POST["drop"]?substr(ME,0,-1):$A),lang(193),$se,lang(194),$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;$K["materialized"]=($kf!="VIEW");if(!$n)$n=adminer_error();}page_header(($a!=""?lang(41):lang(195)),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
adminer_redirect($_);}page_header($fa!=""?lang(216).": ".h($fa):lang(217),$n);if(!$K)$K["name"]=$fa;echo'
<form action="" method="post">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".lang(68).adminer_doc_link(array('sql'=>"grant.html#priv_level"));$s=0;foreach($bd
...
';}elseif(isset($_GET["replication"])){page_header(lang(110));echo"<h3>".lang(240).adminer_doc_link(array("sql"=>"show-master-status.html"))."</h3>\n";$ie=replication_status("MASTER");if(!$ie)echo"<p class='message'>".lang(12)."\n";else{echo"<table cellspacing='0'>\n";foreach($ie[0]as$y=>$X){echo"<tr>","<th>".h($y),"<td>".nbsp($X);}echo"</table>\n";}$Ng=replication_status("SLAVE");if($Ng){echo"<h3>".lang(241).adminer_doc_link(array("sql"=>"show-slave-status.html"))."</h3>\n";foreach($Ng[0]as$Mg){echo"<table cellspacing='0'>\n";foreach($Mg
