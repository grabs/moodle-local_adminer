...
get_rows($H,$i=null,$m="<p class='error'>"){global$h;$rb=(is_object($i)?$i:$h);$J=array();$I=$rb->query($H);if(is_object($I)){while($K=$I->fetch_assoc())$J[]=$K;}elseif(!$I&&!is_object($i)&&$m&&defined("PAGE_HEADER"))echo$m.adminer_error()."\n";return$J;}function
...
adminer_redirect($A,$je=null){if($je!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($A!==null?$A:$_SERVER["REQUEST_URI"]))][]=$je;}if($A!==null){if($A=="")$A=".";header("Location: $A");exit;}}function
query_redirect($H,$A,$je,$Of=true,$tc=true,$Ac=false,$eh=""){global$h,$m,$b;if($tc){$Dg=microtime(true);$Ac=!$h->query($H);$eh=adminer_format_time($Dg);}$Bg="";if($H)$Bg=$b->messageQuery($H,$eh);if($Ac){$m=adminer_error().$Bg;return
false;}if($Of)adminer_redirect($A,$je.$Bg);return
true;}function
...
switch_lang(){global$ca,$Md;echo"<form action='' method='post'>\n<div id='lang'>",lang(19).": ".html_select("lang",$Md,$ca,"this.form.submit();")," <input type='submit' value='".lang(20)."' class='hidden'>\n","<input type='hidden' name='token' value='".get_token()."'>\n";echo"</div>\n</form>\n";}if(isset($_POST["lang"])&&verify_token()){cookie("adminer_lang",$_POST["lang"]);$_SESSION["lang"]=$_POST["lang"];$_SESSION["translations"]=array();adminer_redirect(remove_from_uri());}$ca="en";if(isset($Md[$_COOKIE["adminer_lang"]])){cookie("adminer_lang",$_COOKIE["adminer_lang"]);$ca=$_COOKIE["adminer_lang"];}elseif(isset($Md[$_SESSION["lang"]]))$ca=$_SESSION["lang"];else{$ua=array();preg_match_all('~([-a-z]+)(;q=([0-9.]+))?~',str_replace("_","-",strtolower($_SERVER["HTTP_ACCEPT_LANGUAGE"])),$be,PREG_SET_ORDER);foreach($be
...
as$xd=>$X){if($X[0]<$eh)unset($wd[$xd]);}}$vd=&$wd[$b->bruteForceKey()];if(!$vd)$vd=array($eh+30*60,0);$vd[1]++;$tg=serialize($wd);rewind($Pc);fwrite($Pc,$tg);ftruncate($Pc,strlen($tg));flock($Pc,LOCK_UN);fclose($Pc);}$Ja=$_POST["auth"];if($Ja){$wd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$vd=$wd[$b->bruteForceKey()];$_e=($vd[1]>30?$vd[0]-time():0);if($_e>0)auth_error(lang(72,ceil($_e/60)));session_regenerate_id();$l=$Ja["driver"];$N=$Ja["server"];$V=$Ja["username"];$G=(string)$Ja["password"];$k=$Ja["db"];set_password($l,$N,$V,$G);$_SESSION["db"][$l][$N][$V][$k]=true;if($Ja["permanent"]){$x=base64_encode($l)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($k);$Bf=$b->permanentLogin(true);$qf[$x]="$x:".base64_encode($Bf?encrypt_string($G,$Bf):"");cookie("adminer_permanent",implode(" ",$qf));}if(count($_POST)==1||DRIVER!=$l||SERVER!=$N||$_GET["username"]!==$V||DB!=$k)adminer_redirect(auth_url($l,$N,$V,$k));}elseif($_POST["logout"]){if($bd&&!verify_token()){page_header(lang(71),lang(73));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();adminer_redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(74));}}elseif($qf&&!$_SESSION["pwds"]){session_regenerate_id();$Bf=$b->permanentLogin();foreach($qf
...
<td><input type="radio" name="auto_increment_col" value=""><acronym title="',lang(56),'">AI</acronym>',adminer_doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td>',lang(93),(support("comment")?"<td".($pb?"":" class='hidden'").">".lang(94):"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".lang(95)."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
...
as$C=>$n){$X=process_input($n);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($Fh){if(!$O)adminer_redirect($A);queries_redirect($A,lang(152),$l->update($a,$O,$Kf,!$Ah));if(is_ajax()){page_headers();page_messages($m);exit;}}else{$I=$l->insert($a,$O);$Od=($I?last_id():0);queries_redirect($A,lang(153,($Od?" $Od":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($o
...
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$K["collation"]))$Jg=false;$Nd=$k;}}queries_redirect(ME."db=".urlencode($Nd),lang(169),$Jg);}}else{if(!$K["collation"])adminer_redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),lang(170));}}page_header(DB!=""?lang(59):lang(171),$m,array(),h(DB));$kb=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$kb);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$Tc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Tc,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($kb?html_select("collation",array(""=>"(".lang(85).")")+$kb,$K["collation"]).adminer_doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
...
adminer_redirect($_);}}page_header($_GET["ns"]!=""?lang(60):lang(61),$m);if(!$K)$K["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
<p>
',lang(87),': ',html_select("on_delete",array(-1=>"")+explode("|",$Ke),$K["on_delete"]),' ',lang(86),': ',html_select("on_update",array(-1=>"")+explode("|",$Ke),$K["on_update"]),adminer_doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="',lang(14),'">
...
adminer_redirect($_);}page_header($fa!=""?lang(209).": ".h($fa):lang(210),$m);if(!$K)$K["name"]=$fa;echo'
<form action="" method="post">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".lang(63).adminer_doc_link(array('sql'=>"grant.html#priv_level"));$r=0;foreach($Uc
