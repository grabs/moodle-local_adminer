...
get_rows($H,$i=null,$m="<p class='error'>"){global$h;$qb=(is_object($i)?$i:$h);$J=array();$I=$qb->query($H);if(is_object($I)){while($K=$I->fetch_assoc())$J[]=$K;}elseif(!$I&&!is_object($i)&&$m&&defined("PAGE_HEADER"))echo$m.adminer_error()."\n";return$J;}function
...
adminer_redirect($A,$he=null){if($he!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($A!==null?$A:$_SERVER["REQUEST_URI"]))][]=$he;}if($A!==null){if($A=="")$A=".";header("Location: $A");exit;}}function
query_redirect($H,$A,$he,$Kf=true,$uc=true,$Bc=false){global$h,$m,$b;$Zg="";if($uc){$zg=microtime(true);$Bc=!$h->query($H);$Zg="; -- ".adminer_format_time($zg,microtime(true));}$xg="";if($H)$xg=$b->messageQuery($H.$Zg);if($Bc){$m=adminer_error().$xg;return
false;}if($Kf)adminer_redirect($A,$he.$xg);return
true;}function
queries($H=null){global$h;static$Ef=array();if($H===null)return
implode("\n",$Ef);$zg=microtime(true);$J=$h->query($H);$Ef[]=(preg_match('~;$~',$H)?"DELIMITER ;;\n$H;\nDELIMITER ":$H)."; -- ".adminer_format_time($zg,microtime(true));return$J;}function
...
adminer_format_time($zg,$jc){return
...
switch_lang(){global$ca,$Kd;echo"<form action='' method='post'>\n<div id='lang'>",lang(9).": ".html_select("lang",$Kd,$ca,"this.form.submit();")," <input type='submit' value='".lang(10)."' class='hidden'>\n","<input type='hidden' name='token' value='".get_token()."'>\n";echo"</div>\n</form>\n";}if(isset($_POST["lang"])&&verify_token()){cookie("adminer_lang",$_POST["lang"]);$_SESSION["lang"]=$_POST["lang"];$_SESSION["translations"]=array();adminer_redirect(remove_from_uri());}$ca="en";if(isset($Kd[$_COOKIE["adminer_lang"]])){cookie("adminer_lang",$_COOKIE["adminer_lang"]);$ca=$_COOKIE["adminer_lang"];}elseif(isset($Kd[$_SESSION["lang"]]))$ca=$_SESSION["lang"];else{$ua=array();preg_match_all('~([-a-z]+)(;q=([0-9.]+))?~',str_replace("_","-",strtolower($_SERVER["HTTP_ACCEPT_LANGUAGE"])),$Zd,PREG_SET_ORDER);foreach($Zd
as$B)$ua[$B[1]]=(isset($B[3])?$B[3]:1);arsort($ua);foreach($ua
as$x=>$Df){if(isset($Kd[$x])){$ca=$x;break;}$x=preg_replace('~-.*~','',$x);if(!isset($ua[$x])&&isset($Kd[$x])){$ca=$x;break;}}}$mh=&$_SESSION["translations"];if($_SESSION["translations_version"]!=-1171566891){$mh=array();$_SESSION["translations_version"]=-1171566891;}function
...
long2str($W,true);}$h='';$cd=$_SESSION["token"];if(!$cd)$_SESSION["token"]=rand(1,1e6);$T=get_token();$mf=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($x)=explode(":",$X);$mf[$x]=$X;}}$Ja=$_POST["auth"];if($Ja){session_regenerate_id();$Tb=$Ja["driver"];$N=$Ja["server"];$V=$Ja["username"];$G=$Ja["password"];$l=$Ja["db"];set_password($Tb,$N,$V,$G);$_SESSION["db"][$Tb][$N][$V][$l]=true;if($Ja["permanent"]){$x=base64_encode($Tb)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($l);$yf=$b->permanentLogin(true);$mf[$x]="$x:".base64_encode($yf?encrypt_string($G,$yf):"");cookie("adminer_permanent",implode(" ",$mf));}if(count($_POST)==1||DRIVER!=$Tb||SERVER!=$N||$_GET["username"]!==$V||DB!=$l)adminer_redirect(auth_url($Tb,$N,$V,$l));}elseif($_POST["logout"]){if($cd&&!verify_token()){page_header(lang(62),lang(63));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();adminer_redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(64));}}elseif($mf&&!$_SESSION["pwds"]){session_regenerate_id();$yf=$b->permanentLogin();foreach($mf
...
<td><input type="radio" name="auto_increment_col" value=""><acronym title="',lang(47),'">AI</acronym>',adminer_doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td>',lang(100),(support("comment")?"<td".($ob?"":" class='hidden'").">".lang(101):"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.2' alt='+' title='".lang(102)."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
...
as$l){if(count($k)==1||$l!=""){if(!create_database($l,$K["collation"]))$Eg=false;$Ld=$l;}}queries_redirect(ME."db=".urlencode($Ld),lang(166),$Eg);}}else{if(!$K["collation"])adminer_redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),lang(167));}}page_header(DB!=""?lang(50):lang(168),$m,array(),h(DB));$jb=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$jb);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$Uc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Uc,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($jb?html_select("collation",array(""=>"(".lang(92).")")+$jb,$K["collation"]).adminer_doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
...
adminer_redirect($_);}}page_header($_GET["ns"]!=""?lang(51):lang(52),$m);if(!$K)$K["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
<p>
',lang(94),': ',html_select("on_delete",array(-1=>"")+explode("|",$Ge),$K["on_delete"]),' ',lang(93),': ',html_select("on_update",array(-1=>"")+explode("|",$Ge),$K["on_update"]),adminer_doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="',lang(146),'">
...
adminer_redirect($_);}page_header($fa!=""?lang(206).": ".h($fa):lang(207),$m);if(!$K)$K["name"]=$fa;echo'
<form action="" method="post">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".lang(54).adminer_doc_link(array('sql'=>"grant.html#priv_level"));$r=0;foreach($Vc
